...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['login_btn'])) {
        ...
        $exist_user = "SELECT * FROM adminmaster WHERE admin_email = ? AND password = ? ";
        $exist_user_stmt = $conn->prepare($exist_user);
        $exist_user_stmt->bind_param("ss", $admin_email, $admin_password);
        ...
        if ($result->num_rows === 1) {
            ...
            $_SESSION['message'] = ["sucess" => false, "error_msg" => 'admin not found'];
        }
    }
}
?>
        ...
        <div style="background-color: #1D2129; padding: 32px; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1); border: 1px solid #4B5563;">
            ...
            <form id="loginForm" method="post" accept="#">
                ...
                <div style="margin-bottom: 24px;">
                    ...
                    <input type="password" id="password" name="password" required style="width: 100%; padding: 12px 16px; background-color: #242730; border: 1px solid #4B5563; border-radius: 8px; outline: none; transition: all 0.2s ease-in-out;">
                </div>

                <button type="submit" name="login_btn" style="width: 100%; background-color: #3B82F6; color: #FFFFFF; font-weight: 700; padding: 12px 16px; border-radius: 8px; border: none; cursor: pointer; transition: background-color 0.2s ease-in-out;">
                    Login
                </button>
            </form>
