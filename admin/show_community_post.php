...
$post_status = ["pending", "accepted", "rejected", "disabled"];
...
// disable the post from records
if (isset($_GET['post_id']) && $_GET["fn"] == "dis") {
    $post_id = (int) $_GET["post_id"];

    $sql = "UPDATE community_posts SET status = 'disabled' WHERE post_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $post_id);

    if ($stmt->execute()) {
        $message = "Post diabled successfully";
        header("Location: show_community_post.php?success=" . urlencode($message));
        exit();
    }

    $error = "Post not disabled";
    header("Location: show_community_post.php?error=" . urlencode($error));
    exit();
}

// enable the post from records
if (isset($_GET['post_id']) && $_GET["fn"] == "acc") {
    $post_id = (int) $_GET["post_id"];

    $sql = "UPDATE community_posts SET status = 'accepted' WHERE post_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $post_id);
                            ...
                            <div class="post-card">
                                ...
                                <form action="alumni_post_view.php" method="get">
                                    <div class="post-footer">
                                        <?php if ($pdetails["status"] == "accepted"): ?>
                                            <a href="show_community_post.php?post_id=<?= htmlspecialchars($pdetails["post_id"]) ?>&fn=<?= htmlspecialchars("dis") ?>" class="delete-btn">Disable</a>
                                        <?php else: ?>
                                            <a href="show_community_post.php?post_id=<?= htmlspecialchars($pdetails["post_id"]) ?>&fn=<?= htmlspecialchars("acc") ?>" class="accept-btn">Enable</a>
                                        <?php endif; ?>
                                    </div>
                                </form>
                            </div>
