...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['excel_file'])) {
    ...
    try {
        ...
        $sql = "INSERT INTO alumni_student_master
                    (email, enrollment_no, passout_year, branch)
                VALUES (?, ?, ?, ?)
        ...
        $email = $enrollmentNo = $passoutYear = $branch = "";
        mysqli_stmt_bind_param($stmt, "ssis",$email, $enrollmentNo, $passoutYear, $branch);
        ...
        for ($i = 1; $i < count($rows); $i++) {
            ...
            $passoutYear  = trim($row[2] ?? '');
            $branch       = trim($row[3] ?? '');

            if (empty($email) || empty($enrollmentNo)) {
